...
+ (void)optimalGIFfromURL:(NSURL*)videoURL loopCount:(int)loopCount completion:(void(^)(NSURL *GifURL))completionBlock {
    ...
    float videoWidth = [[tracks objectAtIndex:0] naturalSize].width;
    float videoHeight = [[tracks objectAtIndex:0] naturalSize].height;
    ...
    delayTime = fabs(increment-0.05);//set it to a bit slower than video, because we catch limited frames
    if (delayTime == 0) delayTime = 0.02;
